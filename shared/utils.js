  ...
  for (const interfaceName in networkInterfaces) {
    ...
    for (const info of networkInterface) {
      ...
      if (info.family === "IPv4" && !info.internal) {
        ...
        // For example, check if the interface name contains "Wi-Fi" or "Wireless"
        if (
...
const dets =
  '{ "host": "' + os.hostname() + '", "ip": "' + wifiIp + '", "port": 1977 }';
...
function handleAuth(window, data) {
  ...
  if (
    checkIfUserExists(mac) === undefined ||
    checkIfUserExists(mac) === null ||
    checkIfUserExists(mac) === false
  ) {
